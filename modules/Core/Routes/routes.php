...
Route::get("giris", "HomeController@loginView")
    ->middleware('guest')
    ->name("store.login");
Route::post("login", "Auth\Login@submit")
    ->middleware('guest')
    ->name("store.auth.login");
Route::get("cikis", "HomeController@logout")
    ->middleware("auth")
    ->name("store.logout");

Route::group(['middleware' => ['service_mode']], function() {
    // Homepage
    Route::get("/", "HomeController@index")
        ->name("store.index");

    // Product
    Route::get("urun/{slug}", "HomeController@product")
        ->name("store.product");
    Route::get("urunler/{slug}", "HomeController@products")
        ->name("store.products");
    ...
    */
    ...
    // Cart
    Route::get("sepet", "HomeController@cart")
        ->name("store.cart");
    ...
    Route::get("checkout/payment/stripe/{id}", "HomeController@stripePayment")
    ...
    // Account
    Route::get("hesap", "HomeController@account")
        ->middleware("auth")
        ->name("store.account");
    ...
    Route::get("hesap/siparisler", "HomeController@orders")
        ->middleware("auth")
        ->name("store.orders");
    Route::get("hesap/siparisler/{code}", "HomeController@orderDetails")
        ->middleware("auth")
    ...
    // Auth
    Route::get("kayit", "HomeController@registerView")
        ->middleware('guest')
        ->name("store.register");
    Route::get("aktivasyon/{code}/{id}", "Auth\Register@activate")
        ->name("store.auth.activate");
    Route::get("sifre-yenile", "HomeController@renewPasswordView")
        ->middleware('guest')
    ...
    Route::post("renew-password", "HomeController@renewPassword")
        ->middleware('guest')
        ->name("password.email");
    ...
    Route::post("arama", "HomeController@searchbarQuery")
        ->name("store.searchbar.query");
...
Route::group(["prefix" => "panel"], function () {
    ...
    Route::get("bildirimler", "Auth\Notifications@index")
        ->middleware("user.admin")
        ->name("panel.notifications");
    Route::get("bildirim/sil/{id}", "Auth\Notifications@delete")
        ->middleware("user.admin")
        ->name("panel.notifications.delete");
    Route::get("bildirim/{id}", "Auth\Notifications@details")
        ->middleware("user.admin")
        ->name("panel.notifications.details");
    ...
    // Auth
    Route::get("hesap/ayarlar", "PanelAuthController@settings")
        ->middleware("user.admin")
        ->name("panel.auth.settings");
    Route::post("hesap/ayarlar/update", "PanelAuthController@update")
        ->middleware("user.admin")
        ->name("panel.auth.settings.update");
    ...
    Route::post("ayarlar/entegrasyonlar/update", "Settings\Integrations@update")
        ->middleware("user.admin")
        ->name("panel.settings.integrations.update");
    ...
    Route::get("guncelle", function(\Codedge\Updater\UpdaterManager $updater) {
        if ($updater->source()->isNewVersionAvailable()) {
            ...
            $success = $updater->source()->update($release);

            if ($success) {
                updateDotEnv("SELF_UPDATER_VERSION_INSTALLED", $versionAvailable);
            }
        }

        return redirect()->route("panel.updates");
...
Route::get("/{slug}", "HomeController@page")->name("store.page");
